...
frmLoad.Show
mainConnect.Open
'Suppliers id start from 1 --->
tmpDT = dtDate.Year & "-" & dtDate.Month & "-" & dtDate.Day & Format(dtTime.Value, " hh:mm:ss")
sql(0) = "INSERT INTO purchase(supplierId,dateTime,comment,user) VALUES('" & (cmbSup.ListIndex + 1) & "','" & tmpDT & "','" & txtCom.Text & "','" & username & "')"
sql(1) = "SELECT LAST_INSERT_ID()" 'need to get the purchase id assigned by the server 'cmbSup has it all sorted out in the correct order
mainConnect.Execute sql(0)
records(0).Open sql(1), mainConnect
purchaseId = records(0).fields(0) 'for later reference
records(0).Close

For i = 0 To UBound(cartProd)
If Not (cartProd(i).id = "-1") Then 'The cart product was deleted
   If cartProd(i).type = "NS" Then
   If cartNsProd(cartProd(i).id).id = "" Then 'new NS Product !
   sql(0) = "INSERT INTO nsproduct(name,quantity) VALUES('" & cartProd(i).name & "','" & cartNsProd(cartProd(i).id).quantity & "')"
   mainConnect.Execute sql(0)
   records(0).Open sql(1), mainConnect
   nsProductId = records(0).fields(0) 'for later reference
   records(0).Close
   sql(0) = "INSERT INTO purchase_nsprod(purchaseId,nsProductId,quantity,comment) VALUES('" & purchaseId & "','" & nsProductId & "','" & cartNsProd(cartProd(i).id).quantity & "','" & cartNsProd(cartProd(i).id).comment & "')"
   mainConnect.Execute sql(0)
   Else
   ...
   mainConnect.Execute sql(0)
   sql(0) = "INSERT INTO purchase_nsprod(purchaseId,nsProductId,quantity,comment) VALUES('" & purchaseId & "','" & cartNsProd(cartProd(i).id).id & "','" & cartNsProd(cartProd(i).id).quantity & "','" & cartNsProd(cartProd(i).id).comment & "')"
   mainConnect.Execute sql(0)
   End If
   Else
   If cartProd(i).type = "S" Then
   sql(0) = "INSERT INTO sproduct(name,serial,accessories,supplierId) VALUES('" & cartProd(i).name & "','" & cartSProd(cartProd(i).id).serial & "','" & cartSProd(cartProd(i).id).accessories & "','" & (cmbSup.ListIndex + 1) & "')"
   mainConnect.Execute sql(0)
   records(0).Open sql(1), mainConnect
   sProductId = records(0).fields(0)
   records(0).Close
   sql(0) = "INSERT INTO purchase_sprod(purchaseId,sProductId,comment) VALUES('" & purchaseId & "','" & sProductId & "','" & cartSProd(cartProd(i).id).comment & "')"
   mainConnect.Execute sql(0)
   End If
   End If
End If
...
Load_SProd:
records(0).Close
records(0).Open "SELECT DISTINCT name FROM sproduct", mainConnect 'returns only the table of distinct names !!!!
If records(0).RecordCount = 0 Then GoTo Load_End
ReDim Preserve sProd(records(0).RecordCount - 1)
tmpC = 0
Do Until records(0).EOF
sProd(tmpC).name = records(0).fields(0)
...
If txtSProdSerial.Text = "" Or txtSProdSerial.Text = "Product Serial" Then GoTo Product_Serial_Error
cartProdCount = cartProdCount + 1
